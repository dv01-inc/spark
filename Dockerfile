...
RUN tar -xvzf apache-maven-3.5.3-bin.tar.gz
RUN mkdir -p /usr/share/maven
RUN mv apache-maven-3.5.3/* /usr/share/maven
RUN PATH=$PATH:/usr/share/maven/bin

WORKDIR $TMP

COPY . $TMP

RUN ./dev/make-distribution.sh --name dv01 -Psparkr -Phadoop-2.7 -e \
  && mkdir -p $SPARK_HOME \
