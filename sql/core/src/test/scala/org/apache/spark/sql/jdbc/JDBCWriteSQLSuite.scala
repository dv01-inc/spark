...
class JDBCWriteSQLSuite extends SharedSQLContext with BeforeAndAfter {

  val url1 = "jdbc:sqlserver://md00.dv01.co;databaseName=dv01"
  var conn1: java.sql.Connection = null
  val properties = new Properties()
  properties.setProperty("sqlserverTableLock", "true")
  properties.setProperty("sqlserverTimeout", "6000")
  properties.setProperty("batchSize", "200000")
  properties.setProperty("user", "dv01_pipeline_admin")
  properties.setProperty("password", "sdfRF#!ls_5q1$zxC")
  ...
  test("test") {
    Utils.classForName("com.microsoft.sqlserver.jdbc.SQLServerDriver")
    val CORES = 8L

    val s = SparkSession.builder().master(s"local[$CORES]")
      .appName("dean-local-job")
      .config("spark.ui.enabled", "true")
      .config("spark.sql.shuffle.partitions", CORES)
      .config("spark.eventLog.enabled", "true")
      .getOrCreate()

    val hadoopContext = s.sparkContext.hadoopConfiguration
    hadoopContext.set("fs.azure", "org.apache.hadoop.fs.azure.NativeAzureFileSystem")
    hadoopContext.set(
      "fs.azure.account.key.dv01files.blob.core.windows.net",
      "vUayiAcx9SZmGgvcjtRqwixM3x7wveEWYgoig/O+5wez+czYJ7xWfULQJjYLZ6bOGbacgNDL5HCUnPARqBBT3A=="
    )

    s.sparkContext.setLogLevel("WARN")

    val df2 = s.read.parquet("/data/dv01-pipeline-csvs/promise_financial/promise_financial/daily_parquet_rebuild")
    df2.write.mode(SaveMode.Overwrite).format("jdbc")
          .option("url", url1)
          .option("dbtable", "test.promise")
...
//        |OPTIONS (url '$url1', dbtable 'TEST.PEOPLE', user 'sa', password 'Washington!000')
...
//        |USING org.apache.spark.sql.jdbc
//        |OPTIONS (url '$url1', dbtable 'TEST.PEOPLE1', user 'sa', password 'Washington!000')
//      """.stripMargin.replaceAll("\n", " "))
//  }
//
//  after {
